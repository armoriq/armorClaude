...
| Phase | What | Effort  | Sudo Protection |
|---|---|---|---|---|
| **Phase 0** (NOW) | Ship current architecture as v0.3  | Working policy, 195 tests, immediate value | None (same user) |
| **Phase 1** | Create armoriq-policyd repo, extract daemon  | Standalone daemon with socket API | DAC (separate user) |
| **Phase 2** | System service installer (launchd + systemd)  | OS-level isolation, _armoriq user | DAC + sudo stripping + capabilities |
| **Phase 3** | ArmorClaude thin client migration | ArmorClaude becomes ~200 lines | (inherits Phase 2) |
| **Phase 4** | Socket auth hardening + MAC profiles  | Peer creds, nonce; SELinux/AppArmor/macOS sandbox | **DAC + MAC + kernel lockdown** |
| **Phase 5** | Cross-product client library  | @armoriq/policy-client npm + Python packages | (inherits Phase 4) |
| **Phase 6** | eBPF/Tetragon integration (optional)  | Runtime authority monitoring | Kernel-level observability |
| **Phase 7** (FUTURE) | KAP integration | PAP-KAP + IAP-KAP adapters, kernel enforcement | **Full kernel enforcement** |
