...
test("seedBuiltinProfiles does not overwrite existing profiles", async () => {
  ...
  assert.equal(profile.policy.rules, undefined);
  assert.equal(profile.rules, undefined);
  assert.equal(profile.policy.statements.length, 1);
});
...
test("saveProfile creates a new user profile", async () => {
  ...
  assert.equal(saved.policy.rules, undefined);
  assert.equal(saved.rules, undefined);
  assert.equal(saved.policy.statements[0].id, "p1");
});
...
test("/armor policy profile save saves current policy", async () => {
  ...
  assert.ok(out.includes("2 statements"));

  const profile = await loadProfile(config, "my-setup");
  assert.equal(profile.policy.rules, undefined);
  assert.equal(profile.rules, undefined);
  assert.equal(profile.policy.statements.length, 2);
});
